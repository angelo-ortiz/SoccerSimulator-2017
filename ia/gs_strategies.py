...
from .actions import dribble, shoot, clearSolo, goToBall, goTo, goToMyGoal, interceptBall, \
    goalControl, passBall, goForwardsPA
...
class DribblerTestStrategy(Strategy):
    ...
    def compute_strategy(self, state, id_team, id_player):
        me = StateFoot(state, id_team, id_player)
        if has_ball_control(me):
            return goForwardsPASolo(me, self.alpha, self.beta, self.theta, self.power)
        if is_close_ball(me):
            return get_empty_strategy()
        return goToBall(me)
